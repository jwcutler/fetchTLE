...
/*
Parses and validates TLE lines and loads them into *tle.

Returns 0 on success or -1 on failure. 
*/
int Input_Tle_Set(char *name, char *line_1, char *line_2, tle_t *tle){
    // Setup
    int temp_count, chr, tle_set_count;
    char tle_set[139]; // The two lines of the TLE set
    
    // Load in the TLE satellite name
    temp_count = 0;
    while (name[temp_count] != '\0'){
        tle->sat_name[temp_count++] = name[temp_count];
    }
    tle->sat_name[temp_count] = '\0';
    
    // Load in the first TLE line
    temp_count = 0;
    tle_set_count = 0;
    while (line_1[temp_count] != '\0'){
        tle_set[tle_set_count++] = line_1[temp_count++];
    }
    
    // Load in the second TLE line
    temp_count = 0;
    while (line_2[temp_count] != '\0'){
        tle_set[tle_set_count++] = line_2[temp_count++];
    }
    tle_set[tle_set_count] = '\0';
